...
class AttentionBlock(torch.nn.Module):
    ...
    def forward(self, hidden_states, time_emb, text_emb, res_stack):
        ...
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, encoder_hidden_states=text_emb)
...
class BlockUNet(ModelMixin):
    def __init__(self):
        ...
        self.conv_norm_out = torch.nn.GroupNorm(
            num_channels=320, num_groups=32, eps=1e-5
        )
        ...
        self.config = FrozenDict(
            [
                ...
                (
                    "_name_or_path",
                    "models/stabilityai/stable-diffusion-xl-base-1.0\\unet",
                ),
            ]
        )
        self.add_embedding = FrozenDict(
            [("linear_1", FrozenDict([("in_features", 2816)]))]
        )
    ...
    def process(
        self, sample, timestep, encoder_hidden_states, added_cond_kwargs, **kwargs
    ):
        # 1. time
        ...
        add_embeds = torch.concat(
            [added_cond_kwargs["text_embeds"], time_embeds], dim=-1
        )
    ...
    def forward(
        ...
        for i in range(sample.shape[0]):
            added_cond_kwargs_ = {}
            added_cond_kwargs_["text_embeds"] = added_cond_kwargs["text_embeds"][
                i : i + 1
            ]
