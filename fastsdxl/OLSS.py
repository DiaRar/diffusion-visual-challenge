...
class OLSSSolver:
    ...
    @torch.no_grad()
    def resolve_diffusion_process(
        self, steps_accelerate, t_path, x_path, e_path, i_path=None
    ):
        steps_inference = t_path.shape[0]
        # accelerate path
        if i_path is None:
            i_path = torch.arange(
                0, steps_inference, steps_inference // steps_accelerate
            )[:steps_accelerate]
    ...
    def search_next_step_with_error_limit(
        self, x_prev, e_prev, x_flat, i_lowerbound, max_error
    ):
        i_next = i_lowerbound
    ...
    def search_path_with_error_limit(
        self, max_steps, t_path, x_path, e_path, max_error
    ):
        ...
        for step in range(max_steps):
            ...
            i_next = self.search_next_step_with_error_limit(
                x_prev, e_prev, x_flat, i_lowerbound, max_error
            )
    ...
    @torch.no_grad()
    def resolve_diffusion_process_graph(
        self, num_accelerate_steps, t_path, x_path, e_path, max_iter=30, verbose=0
    ):
        error_l, error_r = 0.0, 10.0
        for it in tqdm(range(max_iter), desc="OLSS is solving the parameters"):
            error_m = (error_l + error_r) / 2
            path = self.search_path_with_error_limit(
                num_accelerate_steps, t_path, x_path, e_path, error_m
            )
            if path is None:
                error_l = error_m
            else:
        ...
        path = self.search_path_with_error_limit(
            num_accelerate_steps, t_path, x_path, e_path, error_r
        )
        timesteps, wx, we = self.resolve_diffusion_process(
            num_accelerate_steps, t_path, x_path, e_path, i_path=path
        )
